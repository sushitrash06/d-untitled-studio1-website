...
export async function GET(
  ...
  try {
    ...
    if (!res.ok) {
      return NextResponse.json({ error: `Project not found in remote API: ${res.statusText}` }, { status: 404 });
    }

    const proj: ExternalProject = await res.json();
    ...
    console.error(`Error fetching/mapping project ${id}:`, error);
